...
export default function AdminDashboard() {
  ...
  // const statsData = [
  //   { title: "Total Santri", value: "48", change: "+4 this month", icon: <FiUsers className="w-6 h-6" />, iconBg: "bg-blue-50", iconColor: "text-blue-600" },
      ...
      {/* <div className="grid grid-cols-1 lg:grid-cols-3 gap-6"> */}
        ...
        {/* <div className="bg-white rounded-2xl border border-blue-100 p-6 shadow-sm">
          <h3 className="font-semibold text-gray-800 text-lg mb-4 flex items-center gap-2">
            ...
            Top Santri This Month
          </h3>
          <div className="space-y-3">
            {topSantri.map((santri) => (
              <div key={santri.rank} className="flex items-center justify-between py-2.5 border-b border-blue-50 last:border-0">
                <div className="flex items-center gap-3">
                  <div className={`w-7 text-center font-bold text-sm ${santri.rank === 1 ? "text-yellow-500" :
                      santri.rank === 2 ? "text-gray-500" :
                        santri.rank === 3 ? "text-amber-700" : "text-gray-400"
                    }`}>
                    {santri.rank === 1 && <FaStar className="w-4 h-4 inline" />}
                    {santri.rank === 2 && <FiStar className="w-4 h-4 inline" />}
                    {santri.rank === 3 && <FiStar className="w-4 h-4 inline" />}
                    {santri.rank > 3 && `#${santri.rank}`}
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-800">{santri.name}</p>
                    <p className="text-xs text-gray-500">{santri.juz} Juz memorized</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-blue-600">{santri.score}%</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-blue-100">
              <th className="text-left py-3 text-gray-500 font-medium text-xs">Santri</th>
          ...
          <tbody className="divide-y divide-blue-50">
            {santriTable.map((santri, idx) => {
              const progressPercent = Math.round(santri.juz / 30 * 100);
              return (
                <tr key={idx} className="hover:bg-blue-50/30 transition-colors">
                  <td className="py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-gray-800">{santri.name}</span>
                    </div>
                  </td>
                  <td className="py-3 text-gray-700">Juz {santri.juz}</td>
                  <td className="py-3 text-gray-500 text-xs">{santri.lastSession}</td>
                  <td className="py-3">
                    <div className="flex items-center gap-2">
                  ...
                  <td className="py-3 hidden sm:table-cell">
                    <span className={`text-xs px-2 py-1 rounded-full ${statusStyles[santri.status as keyof typeof statusStyles]}`}>
                      {santri.status}
                    </span>
                  </td>
                </tr>
